...
pp transient. C8$_M$'s larger tail area ($0.38$ vs.\ C7$_M$'s $0.03$) is the
end-referencing convention of Section~\ref{subsec:metrics} at work: C8$_M$ recovers
to a higher level, so its dip counts in full. C8/C8$_M$ are diagnostic probes: the full buffer defeats the limited-memory
...
\begin{table}[H]
    ...
    \begin{tabular}{l ccc c ccc}
        ...
        \cline{2-4}\cline{6-8}
        $\delta$ & depth (pp)                                             & FORG      & ACC                                   &  & depth (pp) & FORG      & ACC     \\
        \hline
        $1.0$    & $9.0$                                                  & $0.027$   & $0.869$                               &  & $5.7$      & $0.030$   & $0.872$ \\
        $0.3$    & $7.4$                                                  & $0.023$   & $0.873$                               &  & $5.9$      & $0.042$   & $0.867$ \\
        $0.1$    & $5.9$                                                  & $0.018$   & $0.876$                               &  & $6.2$      & $0.057$   & $0.861$ \\
        $0.03$   & $4.8$                                                  & $0.012$   & $0.880$                               &  & $6.7$      & $0.066$   & $0.857$ \\
        \hline
    \end{tabular}
...
\begin{figure}[t]
    ...
    \caption{The full driver ladder: per-step past-task ($T_0$) accuracy across the $T_0 \rightarrow T_1$ transition for the four driver-removal conditions at momentum $0$; solid lines are the seed mean, bands the standard deviation, $x{=}0$ the task switch. The boundary spike falls when balancing switches off the imbalance driver (D1, D2 vs.\ D3, D4), the jitter vanishes when the exact replay gradient switches off the variance driver (D2, D4 smooth), and the smooth dip-and-recover bend that no removal touches, cleanest in D4, is the deterministic arc (Section~\ref{subsec:res_levels}).}
    \label{fig:g_curves}
\end{figure}
