...
\begin{table*}[htbp]
    ...
    \begin{tabular}{@{}p{3.2cm} p{1.2cm} p{1.4cm} p{7.4cm}@{}}
        ...
        \hline
        Overshoot drivers             & \ref{subsec:drivers}    & RQ1       & G1--G4                                                                                 \\
...
\paragraph{Design.} The sweep crosses the damping $\delta \in \{1.0, 0.3, 0.1, 0.03\}$ with two buffer legs, at both momentum settings (Section~\ref{subsec:momentum}). The \emph{standard} leg uses the $1$k reservoir buffer and matches the practical regime of the curriculum sweep. The \emph{full-buffer} leg makes the replay gradient exact, switching off the variance driver as in G2/G4, so the remaining area measures the deterministic arc and the sweep reads out directly whether the directional gate shrinks the arc itself. A solver control (Appendix~\ref{app:impl_per}) confirms the results are not an artefact of an under-converged solve.
