...
The taxonomy yields testable hypotheses about each corner, which the research questions of Section~\ref{sec:proposed_approach} formalise and the experiments then test. The scalar feedback gate is hard to blind-side but slows the new task everywhere, so it should pay a plasticity cost that a separate accelerator must repay. The directional gate should protect the past task at little plasticity cost, but only within what its sampled curvature metric sees; damage arriving through directions the metric rates as flat passes unbraked. The two corners should also interact oppositely with momentum. The curriculum attenuates the push \emph{at source}: with $\lambda \approx 0$ early, momentum finds nothing to accumulate, and by the time $\lambda$ grows the trajectory points the right way, so acceleration helps. The preconditioner attenuates each \emph{step} while the underlying gradient keeps its size, so accumulation across steps can rebuild the push the filter removed, and momentum should partially undo the protection. Sections~\ref{sec:proposed_approach} and~\ref{sec:methods} turn these hypotheses into the research questions and the experimental design.
