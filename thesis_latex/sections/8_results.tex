...
The directional feedforward gate (asymmetric PER) takes a surgical approach: rather than scaling down the entire update like the scalar curriculum, it selectively filters out only the specific gradient components that align with the high-curvature directions of the past task. Table~\ref{tab:res_per} tracks the effect of tightening this filter ($\delta \to 0$) on the standard $1$k buffer, the operating point of every other experiment.
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l cccc}
        ...
        \hline
        no gate  & $19.5 \pm 4.1$ & $6.3$ & $0.873$ & $0.736$             \\
        \hline
        $1.0$    & $9.0 \pm 0.6$  & $3.3$ & $0.869$ & $0.733$             \\
        $0.3$    & $7.4 \pm 1.1$  & $2.3$ & $0.873$ & $0.736$             \\
        $0.1$    & $5.9 \pm 1.5$  & $1.6$ & $0.876$ & $0.739$             \\
        $0.03$   & $4.8 \pm 1.9$  & $1.0$ & $0.880$ & $0.741$             \\
        \hline
    \end{tabular}
    \caption{Asymmetric-PER $\delta$ sweep on the standard ($1$k) buffer at momentum $0$, five seeds, against the ungated reference. Depth and area fall as $\delta \to 0$ while ACC and $\mathrm{WP}_{100}$ \emph{rise}: the directional gate protects the past task at no measured plasticity cost, and every one of these trends holds across all five seeds. Areas are on the step axis ($\eta = 0.1$). The momentum-$0.9$ leg of the same sweep is Figure~\ref{fig:per_curves}(b); the exact-gradient ($60$k) diagnostic leg, the momentum-$0.9$ numbers and the full metric set for every cell are in Appendix~\ref{app:per_full}.}
    \label{tab:res_per}
\end{table}
