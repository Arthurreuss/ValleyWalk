...
The scalar feedback gate (adaptive curriculum) successfully protects past knowledge by globally scaling down the learning rate when boundary gradients threaten stability. Table~\ref{tab:res_curriculum} summarises the key configurations, contrasting the standalone curriculum against its intended momentum-enabled regime.
...
\begin{table*}[htbp]
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tabular}{l ccc ccc}
            ...
            C3: Linear $N{=}200$                 & $14.3 \pm 4.6$                     & $1.8 \pm 0.6$                      & $0.828 \pm 0.025$ & $6.2 \pm 0.6$            & $0.1 \pm 0.0$           & $0.925 \pm 0.007$          \\
            C4: Adaptive                         & $12.8 \pm 2.9$                     & $2.2 \pm 0.5$                      & $0.833 \pm 0.025$ & $2.5 \pm 0.5$            & $0.3 \pm 0.1$           & $0.917 \pm 0.001$          \\
            C7: Adaptive $\lambda_{\min}{=}0.20$ & $13.9 \pm 3.4$                     & $2.7 \pm 0.6$                      & $0.837 \pm 0.031$ & $\mathbf{3.7 \pm 0.4}$   & $\mathbf{0.0 \pm 0.0}$  & $\mathbf{0.931 \pm 0.002}$ \\
            \hline
        \end{tabular}%
    }
    \caption{$\lambda$-curriculum key conditions on rot-MNIST against vanilla ER, at momentum $0.0$ and $0.9$; five seeds: the longest linear ramp (C3), the pure adaptive schedule (C4), and the practical configuration (C7). Ultimately, the combination of the adaptive curriculum and momentum achieves profound stability while fully restoring final accuracy to vanilla levels (C7 at momentum $0.9$, in bold); without momentum the same gate buys its smaller gap with accuracy. The full sweep (C1--C7), with vanilla ER's full metric row as the reference, is in Appendix~\ref{app:curr_full}.}
    \label{tab:res_curriculum}
\end{table*}
...
\begin{table*}[htbp]
    ...
    \resizebox{\textwidth}{!}{%
        \begin{tabular}{l cccc cccc}
            \hline
                     & \multicolumn{4}{c}{Momentum $0.0$} & \multicolumn{4}{c}{Momentum $0.9$}                                                                                                          \\
            $\delta$ & depth (pp)                         & area                              & ACC     & $\mathrm{WP}_{100}$ & depth (pp)     & area   & ACC     & $\mathrm{WP}_{100}$ \\
            \hline
            no gate  & $19.5 \pm 4.1$                     & $6.27$                            & $0.873$ & $0.736$             & $15.9 \pm 2.6$ & $1.03$ & $0.928$ & $0.808$             \\
            \hline
            $1.0$    & $9.0 \pm 0.6$                      & $3.29$                            & $0.869$ & $0.733$             & ---            & ---    & ---     & ---                 \\
            $0.3$    & $7.4 \pm 1.1$                      & $2.32$                            & $0.873$ & $0.736$             & $10.4 \pm 0.5$ & $0.50$ & $0.929$ & $0.813$             \\
            $0.1$    & $5.9 \pm 1.5$                      & $1.56$                            & $0.876$ & $0.739$             & $9.4 \pm 0.5$  & $0.38$ & $0.928$ & $0.815$             \\
            $0.03$   & $\mathbf{4.8 \pm 1.9}$             & $\mathbf{1.04}$                   & $0.880$ & $0.741$             & $8.5 \pm 0.6$  & $0.23$ & $0.929$ & $0.816$             \\
            \hline
        \end{tabular}%
    }
    \caption{Asymmetric-PER $\delta$ sweep on the standard ($1$k) buffer at momentum $0.0$ and $0.9$, five seeds, against the ungated reference. At momentum $0$ depth and area fall as $\delta \to 0$ while ACC and $\mathrm{WP}_{100}$ \emph{rise}, so the directional gate protects the past task at no measured plasticity cost, and every one of these trends holds across all five seeds (strongest filter in bold). At momentum $0.9$ the ordering in $\delta$ survives but the ladder is displaced upward by $3$ to $4$ pp of depth, and what accuracy it reaches vanilla ER already has ($0.929$ against $0.928$): the per-step filter is re-inflated by the accumulator. Areas are on the step axis ($\eta = 0.1$) and fall under momentum for the ungated reference too, so depth is the axis on which the loss of protection reads. No $\delta{=}1.0$ cell was run at momentum $0.9$. The exact-gradient ($60$k) diagnostic leg and the full metric set for every cell are in Appendix~\ref{app:per_full}.}
    \label{tab:res_per}
\end{table*}

\paragraph{Protection at no plasticity cost.}
Figure~\ref{fig:per_curves}(a) illustrates the gate operating in its practical regime. As the directional filter strengthens (moving from the lighter to the darker lines), the \emph{depth} of the boundary dip shrinks monotonically, and the total \emph{area} of forgetting is drastically reduced. Unlike the scalar gate, this surgical intervention leaves safe gradient directions completely untouched, meaning it does not broadly slow down early new-task learning. Consequently, this continuous reduction in gap depth and area is achieved while simultaneously \emph{increasing} final accuracy. The gate effectively recovers the idealized ``arc'' trajectory without requiring microscopic step sizes or incurring a plasticity debt. While the practical $1$k buffer exhibits monotonic protection across all seeds, exact-gradient diagnostic controls ($60$k buffer) and late-stage convergence limits are detailed in Appendix~\ref{app:per_full}.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=\linewidth]{img/results/asymmetric_per/per_sweep.png}
    \caption{The directional feedforward gate on the standard $1$k buffer: per-step past-task ($T_0$) accuracy across the rot-MNIST switch, sweeping the damping $\delta$ (weaker filter light $\rightarrow$ stronger filter dark); seed mean with $\pm$std bands, $x{=}0$ the task switch. \textbf{(a) Momentum $0$:} the boundary dip shrinks monotonically as $\delta \to 0$, and accuracy rises with it (Table~\ref{tab:res_per}), so the protection is bought at no plasticity cost. \textbf{(b) Momentum $0.9$:} the same filters, re-inflated, every rung sitting $3$--$4$ pp deeper than its momentum-$0$ counterpart as the accumulator rebuilds the displacement the per-step filter removes. Both panels span the same $0.27$ of accuracy, so a deeper-looking dip is a deeper dip; the pre-switch level is higher in (b) only because momentum converges $T_0$ further before the switch. The exact-gradient ($60$k) diagnostic leg is in Appendix~\ref{app:per_full}.}
    \label{fig:per_curves}
\end{figure}

\paragraph{Momentum re-inflates what the filter brakes.}
The protection does not survive the accelerator. Repeating the same standard-buffer sweep at momentum $0.9$ (Figure~\ref{fig:per_curves}(b)) moves every rung the wrong way: depth climbs from $7.4$, $5.9$ and $4.8$ pp at $\delta = 0.3$, $0.1$ and $0.03$ to $10.4$, $9.4$ and $8.5$ pp, the whole ladder displaced upward by $3$ to $4$ pp with its ordering intact, so no setting of the filter recovers the momentum-$0$ protection (Table~\ref{tab:res_per}). The mechanism is the per-step locality of the gate. It brakes the push once, at the step it is applied, but momentum accumulates the braked updates and rebuilds the very displacement the filter removed, so what the gate subtracts at one step returns over the next several. Against vanilla ER at the same momentum ($15.9$ pp) the gate still helps, but the reduction it buys falls from ${\approx}75\%$ of vanilla's depth at momentum $0$ to ${\approx}47\%$, and it buys nothing in accuracy that vanilla does not already have ($0.929$ against $0.928$). The area moves the other way, $1.04 \rightarrow 0.23$ at $\delta = 0.03$, as it does for vanilla ER itself ($6.3 \rightarrow 1.0$): momentum shortens every transient, so depth rather than area is the axis on which the loss of protection is visible. This is the failure the scalar gate does not share, and it is what Section~\ref{subsec:res_regimeflip} reads as a regime flip rather than a trade-off.

\subsection{Reading the Momentum Crosses Together}
\label{subsec:res_regimeflip}

When evaluated side-by-side on realistic memory budgets, it becomes clear that the two gates do not sit on a continuous stability-plasticity trade-off curve. Instead, their viability is strictly dictated by the momentum regime.
