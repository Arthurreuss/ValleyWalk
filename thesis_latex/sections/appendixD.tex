...
\end{equation*}
Assuming the lag reaches a steady state, the error stops growing ($\frac{d\delta}{dt} \approx 0$). This is a first-order, gradient-flow approximation: it linearises the gradient around $\Theta^*$ and treats the dynamics as continuous, so it captures the scaling of the lag with $N$ rather than an exact discrete-time bound.
\begin{align*}
...
\end{align*}
From Part A, $L_{\text{replay}}(\Theta^*)$ cannot exceed the final joint optimum loss $L_{\text{replay}}(\theta_{\text{joint}}^*)$. From Part B, the lag $\delta$ is $O(1/N)$. Substituting these limits yields the final bound:
\begin{equation}
