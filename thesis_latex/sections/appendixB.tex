...
\begin{table}[H]
    ...
    \begin{tabular}{@{}p{2.3cm} p{7.9cm} p{4.4cm}@{}}
        ...
        \texttt{gap\_area}       & $\sum_j \int_0^{T} \max\!\left(0,\, b_j - a_j(t)\right) dt$, trapezoidal over the full new-task record (to task end $T$), with $b_j = \min\!\left(a_j^{\text{pre}},\, \bar a_j^{\text{end}}\right)$ and $\bar a_j^{\text{end}}$ the trailing-window mean (last $\max(5, \lceil 0.1 n\rceil)$ samples); accuracy at or above $b_j$ contributes $0$ & Transient \emph{bend}: dip below the recovered level, excluding permanent forgetting and continued learning (units: accuracy $\cdot$ steps) \\
