"""Domain CIFAR-10 continual-learning benchmark.

3 tasks, each applying a different image corruption to every CIFAR-10 image.
All tasks share the same 10 classes — this is a Domain-IL setting (no class
...
Standard CIFAR-10 normalisation is applied after corruption:
...
Usage:
    ...
    dataset = DomainCIFAR10(cfg, batch_size=64)
    train_loader, test_loader = dataset.get_task_loaders(task_id=0)
    # → batches of shape (batch, 3, 32, 32), labels in [0, 9]
"""
...
# ---------------------------------------------------------------------------
# CIFAR-10 normalisation constants (computed from the training set)
# ---------------------------------------------------------------------------
...
class _CorruptedCIFAR10Task(Dataset):
    """Wraps a base CIFAR-10 dataset and applies a fixed corruption + normalise.
    ...
    Args:
        base_dataset:    Torchvision CIFAR10 dataset with transform=None.
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, int]:
        ...
        Returns:
            x:     Float32 tensor, shape (3, 32, 32), normalised to CIFAR-10 stats.
...
class DomainCIFAR10:
    """Domain CIFAR-10 continual-learning dataset manager.

    Loads CIFAR-10 once (no transform) and creates per-task loaders on demand
    by wrapping the base dataset in ``_CorruptedCIFAR10Task``.
    ...
    def __init__(
        ...
        # -----------------------------------------------------------------
        # Load CIFAR-10 once with no transform (raw PIL images).
        # Corruption + normalisation are applied per sample in __getitem__.
        # -----------------------------------------------------------------
        self._train_base = torchvision.datasets.CIFAR10(
            root=self.download_dir,
            train=True,
            download=True,
            transform=None,
        )
        self._test_base = torchvision.datasets.CIFAR10(
            root=self.download_dir,
            train=False,
            download=True,
            transform=None,
    ...
    def get_task_loaders(
        ...
        - ``labels`` has shape ``(batch_size,)``, dtype int64, in [0, 9].

        Args:
            task_id: Zero-based task index in [0, num_tasks).

        Returns:
            train_loader: Shuffled loader over 50 000 CIFAR-10 training images.
            test_loader:  Un-shuffled loader over 10 000 CIFAR-10 test images.
        ...
        train_ds = _CorruptedCIFAR10Task(
            self._train_base, corruption, self.severity, self._normalise
        )
        test_ds = _CorruptedCIFAR10Task(
